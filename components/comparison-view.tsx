...
export default function ComparisonView({ comparison, direction }: ComparisonViewProps) {
  ...
  return (
    <div className="rounded-xl border border-[var(--card-border)] shadow-sm overflow-hidden" style={{ background: "var(--card)" }}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[var(--card-border)] px-5 py-3.5" style={{ background: "var(--subtle-bg)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Behavioral Comparison</h3>
            <p className="text-[11px]" style={{ color: "var(--muted)" }}>Step-by-step mapping from {sourcePlatform} → {targetPlatform}</p>
          </div>
        </div>
...
function StepRow({
  ...
  return (
    <div className={`transition-colors ${isExpanded ? config.bg : ""}`} style={!isExpanded ? { ['--tw-bg-opacity' as string]: 1 } : undefined}>
      {/* Main Row */}
      <button
        onClick={onToggle}
        className="flex w-full items-center gap-3 px-5 py-3 text-left hover:opacity-90 transition-opacity"
      >
        {/* Status indicator */}
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <span className="truncate text-[13px] font-medium" style={{ color: "var(--foreground)" }}>{mapping.sourceStep}</span>
          <span className="flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-mono" style={{ background: "var(--hover-bg)", color: "var(--muted)" }}>
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          {mapping.targetStep ? (
            <Fragment>
              <span className="truncate text-[13px] font-medium" style={{ color: "var(--foreground)" }}>{mapping.targetStep}</span>
      ...
      {isExpanded && (
        <div className="px-5 pb-4 pt-0">
          <div className="ml-9 space-y-2 rounded-lg border p-3" style={{ borderColor: "var(--card-border)", background: "var(--card)" }}>
            <div className="flex items-center gap-2 border-b pb-2 mb-2" style={{ borderColor: "var(--divider)" }}>
