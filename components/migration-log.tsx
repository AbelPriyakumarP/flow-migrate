...
export default function MigrationLog({ logs, isLoading, error, validationIssues = [] }: MigrationLogProps) {
  ...
  return (
    <div className="animate-fadeIn space-y-3">
      {/* Migration Log */}
      <div className="overflow-hidden rounded-xl border border-[var(--card-border)] shadow-sm" style={{ background: "var(--card)" }}>
        <div className="flex items-center gap-2 border-b border-[var(--card-border)] px-4 py-2.5" style={{ background: "var(--subtle-bg)" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14,2 14,8 20,8"/><line x1="16" x2="8" y1="13" y2="13"/><line x1="16" x2="8" y1="17" y2="17"/></svg>
          <span className="text-[13px] font-semibold" style={{ color: "var(--foreground)" }}>Migration Log</span>
        </div>
        <div className="space-y-1 p-3">
          {isLoading && (
            <>
              ...
              {/* Skeleton rows */}
              <div className="skeleton h-8 w-full animate-stagger-1" />
              <div className="skeleton h-8 w-[85%] animate-stagger-2" />
              <div className="skeleton h-8 w-[70%] animate-stagger-3" />
              <div className="skeleton h-8 w-[90%] animate-stagger-4" />
            </>
          )}
          {error && (
      ...
      {hasIssues && (
        <div className="overflow-hidden rounded-xl border border-[var(--card-border)] shadow-sm" style={{ background: "var(--card)" }}>
          <div className="flex items-center gap-2 border-b border-[var(--card-border)] px-4 py-2.5" style={{ background: "var(--subtle-bg)" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="2"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="m9 12 2 2 4-4"/></svg>
            <span className="text-[13px] font-semibold" style={{ color: "var(--foreground)" }}>Schema Validation</span>
            {errors.length > 0 && (
