  ...
  return (
    <div className="card-premium flex flex-wrap items-center gap-5 px-6 py-4">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.12em]" style={{ color: "var(--muted)" }}>Source</p>
          {detection && detection.platform !== "unknown" ? (
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-[14px] font-bold" style={{ color: "var(--foreground)" }}>{detection.label}</p>
            ...
            <p className="text-[13px] mt-0.5" style={{ color: "var(--muted)" }}>Auto-detected from input</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3.5">
        {platformIcon(target)}
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.12em]" style={{ color: "var(--muted)" }}>Target</p>
          <select
            value={target}
            onChange={(e) => onTargetChange(e.target.value as Platform)}
            className="mt-0.5 rounded-md border-none bg-transparent p-0 text-[14px] font-bold outline-none focus:ring-0 cursor-pointer"
