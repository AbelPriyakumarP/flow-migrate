...
function GraphPanel({
  ...
  if (initialNodes.length === 0) {
    return (
      <div className="flex flex-1 items-center justify-center rounded-xl border-2 border-dashed p-8" style={{ borderColor: "var(--card-border)", background: "var(--subtle-bg)" }}>
        <p className="text-sm" style={{ color: "var(--muted)" }}>No workflow to visualize</p>
      </div>
    );
  }

  return (
    <div className="flex flex-1 flex-col rounded-xl border border-[var(--card-border)] shadow-sm overflow-hidden" style={{ background: "var(--card)" }}>
      {/* Panel Header */}
      <div className="flex items-center gap-2 border-b border-[var(--card-border)] px-4 py-2.5" style={{ background: "var(--subtle-bg)" }}>
        <div className="h-2 w-2 rounded-full" style={{ backgroundColor: platformColor }} />
        <span className="text-xs font-semibold" style={{ color: "var(--foreground)" }}>{title}</span>
        <span
...
export default function WorkflowGraphView({
  ...
  return (
    <div className="rounded-xl border border-[var(--card-border)] shadow-sm overflow-hidden animate-fadeIn" style={{ background: "var(--card)" }}>
      {/* Header */}
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="flex w-full items-center justify-between border-b border-[var(--card-border)] px-5 py-3.5 transition-colors" style={{ background: "var(--subtle-bg)" }}
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h3 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Visual Workflow Graph</h3>
      ...
      {isExpanded && (
        <div className="p-4">
          {/* Legend */}
          <div className="mb-3 flex flex-wrap items-center gap-3 rounded-lg px-4 py-2 border" style={{ background: "var(--subtle-bg)", borderColor: "var(--card-border)" }}>
            <span className="text-[10px] font-semibold uppercase tracking-wider mr-1" style={{ color: "var(--muted)" }}>Legend</span>
            {[
          ...
          {mappingLines.length > 0 && (
            <div className="mt-3 rounded-lg border overflow-hidden" style={{ borderColor: "var(--card-border)", background: "var(--subtle-bg)" }}>
              ...
              <div className="divide-y" style={{ borderColor: "var(--divider)" }}>
                {mappingLines.map(line => (
                  <div key={line.sourceStep} className="flex items-center gap-3 px-4 py-1.5">
                    ...
                    />
                    <span className="text-[11px] font-medium min-w-0 flex-1 truncate" style={{ color: "var(--muted)" }}>
                      {line.sourceStep}
                    </span>
