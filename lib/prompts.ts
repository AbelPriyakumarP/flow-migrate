...
- DynamoDB → "type": "Http" (Cosmos DB REST API)
- SQS → "type": "ApiConnection" (Service Bus)
- SNS → "type": "ApiConnection" (Service Bus topic or Event Grid)
- S3 → "type": "Http" (Blob Storage REST API)
- Pass → "type": "Compose"
- Wait → "type": "Wait"
...
Now convert the following AWS Step Functions definition. Output ONLY valid JSON:
`;
...
RULE 7 — TASK RESOURCES:
- Azure Function → "Resource": "arn:aws:lambda:REGION:ACCOUNT:function:<name>"
- Azure Http → Lambda Task
- Azure Cosmos DB → "Resource": "arn:aws:states:::dynamodb:getItem"
- Azure Service Bus → "Resource": "arn:aws:states:::sqs:sendMessage"
